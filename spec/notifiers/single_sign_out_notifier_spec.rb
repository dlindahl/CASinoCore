...
describe CASinoCore::Notifiers::SingleSignOutNotifier do
  ...
  describe '#notify' do
    ...
    it 'sets the timeout values' do
      [:read_timeout=, :open_timeout=].each do |timeout|
        Net::HTTP.any_instance.should_receive(timeout).with(CASinoCore.config.service_ticket[:single_sign_out_notification][:timeout])
      end
      notifier.notify
    end
